...
- `INVERT` contrasts with two backgrounds at once. The widget is painted `LIGHT` and then inverted at every screen pixel whose background is bright, so a card lying across the edge of a bright window is white on the dark side and black on the bright side, with the join following the edge exactly. A widget being dragged paints plain `LIGHT` until it settles, since the last photograph no longer describes what is under it. It needs the extension as well, which hides Sysi for the instant it photographs the desktop; without one it just paints `LIGHT`. A picture pasted into a note comes out as a negative wherever the background is bright — right-click that note and pick `LIGHT` or `DARK` to keep its colours.
- `DICTATE` dims the screen so you can drag out a rectangle. The compositor photographs that region with Sysi hidden, `tesseract -l eng+vie` reads it, and the text lands on the clipboard. It needs `tesseract-ocr` and `tesseract-ocr-vie` installed, and on Wayland it needs the panel extension since only the compositor can see Wayland windows.
...
Xwayland cannot grab a key while a Wayland window is focused, and an Xwayland client cannot take the keyboard from a native Wayland app either. The panel extension therefore grabs `Ctrl+Alt+N` inside the compositor and activates the overlay before Notes opens, so the search field can type from any app. If the extension is off, Sysi falls back to a GNOME custom shortcut (`sysi --panel-action toggle-notes`) that still opens Notes but may leave the caret outside the search field. `Ctrl+Alt+O` still needs an X11 window focused, or bind `sysi --toggle` yourself. The panel strip's `LOCK` / `UNLOCK` and `NOTES` buttons and `Escape` are unaffected.
